## Google Data Analytics Certificate — Practice & Notes
**Breana Palmer** | [LinkedIn](https://linkedin.com/in/breana-palmer) | [GitHub](https://github.com/BreaP)

## About This Repo
This repository documents my progress through the **Google Data Analytics Professional 
Certificate** on Coursera. It includes notes, practice exercises, and 
mini-projects completed throughout the course.

I hold a **Bachelor's degree in Software Development** and am building on that foundation with specialized data analytics training, including hands-on work in Python, SQL, and data visualization. 

## Course Progress

| # | Course | Status |
|:----------:|:----------:|:----------:|
| 1    | Foundations of Data, Data, Everywhere     | ✅ Complete     |
| 2    | Ask Questions to Make Data-Driven Decisions     |  ✅ Complete    |
| 3    | Prepare Data for Exploration     | ✅ Complete     |
| 4    | Process Data from Dirty to Clean     |  ✅ Complete    |
| 5    | Analyze Data to Answer Questions     |  ✅ Complete   |
| 6    | Share Data Through the Art of Visualization     |    ✅ Complete   |
| 7    | Introduction to Data Analysis Using Python     |  ✅ Complete   |
| 8    | Google Data Analytics Capstone Project     | ✅ Complete       |
| 9    | Accelerate Your Job Search with AI     | ✅ Complete       |


## Tools & Skills Being Developed
- **Python** - data manipulation and analysis
- **SQL** - querying and managing databases
- **Google Sheets / Excel** - spreadsheet analysis
- **Tableau** - data visualization and dashboards
- **R** (basic) - statistical analysis
- **Data cleaning & analysis**

## Portfolio Projects
| Project | Tools Used | Description |
|:----------:|:----------:|:----------:|
| Bellabeat Case Study   | Google Sheets     | Analyzed Fitbit fitness tracker data to identify smart device usage trends and provide marketing recommendations for Bellabeat     |
| More coming soon...    |      |      |

## Background
- 🎓 B.S. in Software Development
- 🎖️ U.S. Military Veteran
- 📍 Open to **remote** opportunities
- 💼 Seeking roles in: Data Analytics, Business Intelligence, Junior Development



## Status
🟡 **In Progress** — Started April 2026
✅ **Completed** — Finshed May 2026
